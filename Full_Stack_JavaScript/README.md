...
> You would use the <code>opacity</code> property followed by a value between 0 (invisible) to 1 (visually solid).<br> ><code>opacity</code> is the opposite to transparency.
...
> <code>:nth-child(2n)</code><br> ><code>:nth-child(even)</code>

---

<h4>What is the difference between div:first-child and div:last-child? What will each select?</h4>

> first-child will select the first child of the div, whereas last-child would select the last.<br> ><strong>NOTE:</strong>If there is only one div child then both last and first child would select this child.
...
<details>
<summary><h3>Keys in React</h3></summary>

---

<h4>Why does React need keys?</h4>

> React needs keys because it allows items to be tracked so that changes can be accurately updated and entire lists don't need to be re-rendered.

---

<h4>How do you use keys?</h4>

> You use keys by passing them into the component or DOM element as a prop.

---

<h4>Where should the key value ideally come from?</h4>

> The key value should come from unique id associated with each item.

---

<h4>When can we use an array index as the key value?</h4>

> It is not recommended to use an array index as the key value, but it may be allowable if the list will remain unchanged throughout the applications life.

---

<h4>What is an anti-pattern when using keys?</h4>

> An anti-pattern when using keys to be aware of is that Keys <strong>should never</strong> be generated on the fly. Keys should be inferred from the data itself.
